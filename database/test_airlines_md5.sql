...
FROM country
ORDER BY country_id;
INSERT INTO found_values VALUES ('country', @recs := (SELECT COUNT(*) FROM country), @crc);
...
INSERT INTO found_values VALUES ('manufacturer', @recs := (SELECT COUNT(*) FROM manufacturer), @crc);
...
SELECT @crc := MD5(CONCAT_WS('#', @crc,
       ...
       COALESCE(flight_code,''), COALESCE(aircraft_id,''), COALESCE(arrival_time,''), COALESCE(departure_time,'')))
FROM leg
ORDER BY leg_id;
...
-- --------------------------------------------------
-- Show results (calculated)
-- --------------------------------------------------
SELECT * FROM found_values;

-- --------------------------------------------------
-- If you want to save current found_values as expected (baseline), uncomment:
-- --------------------------------------------------
-- TRUNCATE TABLE expected_values;
-- INSERT INTO expected_values SELECT * FROM found_values;

-- --------------------------------------------------
-- Comparison query: show which tables match the baseline
-- --------------------------------------------------
SELECT e.table_name,
       e.recs AS expected_recs, f.recs AS found_recs,
       e.crc_md5 AS expected_md5, f.crc_md5 AS found_md5,
