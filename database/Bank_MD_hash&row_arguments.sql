/* =======================================================================================================
   File        : validate_table_md5.sql
   Author      : Arava Jagadeeswar Reddy
   Description : Stored Procedure to validate row count and MD5 hash integrity for specified banking tables.
...
BEGIN
    ...
    /* ============================================================
       Step 2: Build MD5 Hash Query based on Table Name
    ============================================================ */
    IF p_table_name = 'account' THEN
        SET @v_sql_hash = CONCAT(
            "SELECT MD5(GROUP_CONCAT(CONCAT_WS('#', account_number, account_type_id, status_id, date_opened, IFNULL(date_closed, 'NULL'), account_type) ORDER BY account_number)) INTO @tmp_hash FROM account"
        );

    ELSEIF p_table_name = 'customer' THEN
        SET @v_sql_hash = CONCAT(
            "SELECT MD5(GROUP_CONCAT(CONCAT_WS('#', customer_id, first_name, last_name, date_of_birth, signup_date) ORDER BY customer_id)) INTO @tmp_hash FROM customer"
        );

    ELSEIF p_table_name = 'customer_account' THEN
        SET @v_sql_hash = CONCAT(
            "SELECT MD5(GROUP_CONCAT(CONCAT_WS('#', customer_id, account_number) ORDER BY customer_id, account_number)) INTO @tmp_hash FROM customer_account"
        );

    ELSEIF p_table_name = 'transaction' THEN
        SET @v_sql_hash = CONCAT(
            "SELECT MD5(GROUP_CONCAT(CONCAT_WS('#', transaction_id, from_account, to_account, transaction_datetime, description, amount) ORDER BY transaction_id)) INTO @tmp_hash FROM transaction"
        );
...
-- Example Call:
CALL validate_table_md5('account', 150, '75f8279733d7c4b227d3471a6ac86921');
